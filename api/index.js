...
app.post("/login", async (req, res) => {
  ...
  app.put("/listings", async (req, res) => {
    const { token } = req.cookies;
    const {
      id, title, city, type, address, images, description,
      perks, checkIn, checkOut, maxGuests, price
    } = req.body;
    jwt.verify(token, jwtSecret, {}, async (err, userData) => {
      if (err) throw err;
      ...
      if (userData.id === listingDoc.user.toString()) {
        const requiredFields = [title, city, type, address, images, description,
          perks, checkIn, checkOut, maxGuests, price];
        if (requiredFields.some(field => !field)) {
          ...
          listingDoc.set({
            user: userData.id,
            title, city, type, address, images, description,
            perks, checkIn, checkOut, maxGuests, price
          });
